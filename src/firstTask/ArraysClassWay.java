...
public class ArraysClassWay {
     ...
     * @return an array of ints filled with random numbers from the interval [4,33]
     */
    public static int[] fill1DArray(int length) {
        Random rand = new Random();
        int[] arr = new int[length];
        for (int i = 0; i < arr.length ; i++) {
            arr[i] = rand.nextInt(4, 34);
        }
        return arr;
    }
     ...
     * @param i an int variable that specifies the row to be searched
     * @return The maximum element in the column
     */
    public static int getMax(int[][] arr, int i) {
        Integer[] row = new Integer[arr[0].length];
    ...
    public void start(int rows, int cols) {
        ...
        int[] arr_of_max = new int[rows];
        String[] s_strArr = new String[rows];
        ...
        Arrays.sort(s_strArr, (s1, s2) -> -Integer.compare(s2.length(), s1.length()));
